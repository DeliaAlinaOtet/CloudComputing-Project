...
$("#meteo-form").submit(function (event) {
    ...
    getMeteoByCity(city, function (data, error){
        if (error == null) {
            ...
            meteoTitle = $('#meteo-title span');
            meteoTitle.html('Oras <span class="text-muted">' + city + '</span> not found');
        }
        // Stop loader
        setTimeout(function () {
            loading.attr('class', 'loading')
...
function displayMeteo(data){
    ...
    $('#meteo-title span').html('Vremea in <a href="' + googleMapCity + '" class="text-muted meteo-city" target="_blank">' + data.city.name + ', ' + data.city.country + '</a>');
